...
export default function NutritionScreen() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={styles.content}>
      <Text style={[styles.title, { color: colors.text }]}>Plani i Ushqimit</Text>
      <Text style={[styles.subtitle, { color: colors.icon }]}>
        Plani juaj i ushqimit nga trajneri
      </Text>
      {hasNutrition ? (
        <View style={[styles.card, { borderColor: `${colors.tint}30` }]}>
          <Text style={[styles.cardTitle, { color: colors.text }]}>Plani aktual</Text>
          <Text style={[styles.cardDesc, { color: colors.icon }]}>Ditët dhe vaktet do të shfaqen këtu.</Text>
        </View>
      ) : (
